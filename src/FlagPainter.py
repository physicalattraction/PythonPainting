...
class FlagPainter(object):
    ...
    def __init__(self, HWratio):
        '''Create an empty (white) image
        
        Inputs:
        -------
        HWratio: height-to-width ratio of the flag
        
        Sets:
        -----
        img: PIL Image, size (W,H). W = 600 pixels, H = HWratio * 600 pixels
    ...
    def stripes(self, colors, ratios, stripe_direction):
        ...
        assert len(colors) == len(ratios), 'Each stripe shall have a color and a ratio'
        ...
        for c, r in zip(colors, ratios):
            ...
            if stripe_direction is StripeDirection.horizontal:
                ...
                y_b += (r / sum(ratios)) * self.H
            elif stripe_direction is StripeDirection.vertical:
                x_l = x_r  # New left is previous right
                x_r += (r / sum(ratios)) * self.W
    ...
    def draw_horizontal_band(self, height, color):
        ...
        assert isinstance(height, tuple), 'Height must be a 2-tuple, is now a {}'.format(type(height))
    ...
    def draw_polygon(self, points, color):
        ...
        for xy in points:
            points_abs.append((int(round(xy[0] * self.W)), int(round(xy[1] * self.H))))
    ...
    def draw_star(self, center, radius_inner, radius_outer, N_points, starting_alpha, color):
        ...
        N_points: number of points the star has. Shall be at least 3
        ...
        assert N_points >= 3, 'Number of points shall be at least 3'
        ...
        r = radius_inner
        HW = self.H / self.W
        Delta_alpha = math.pi / N_points
        for p in range(N_points):
            alpha = 2 * math.pi * p / N_points + starting_alpha
            X1 = X + R * math.cos(alpha)
            Y1 = Y + R * math.sin(alpha) / HW
            X2 = X + r * math.cos(alpha + Delta_alpha)
            Y2 = Y + r * math.sin(alpha + Delta_alpha) / HW
    ...
    def place_drawing(self, img_name, center, size):
        ...
        size: 2-tuple, indicating the relative size  (w.r.t. flag size) of the drawing.
              size = (W,H)
              If W is None and H is not None, the width is determined from H and the drawing size
              If H is None and W is not None, the height is determined from W and the drawing size
              If W and H are not None, the width and height are determined from the drawing size (not recommended)
        '''
        ...
        if size[1] is not None:
            H = size[1] * self.H
        if size[0] is None:
            W = (drawing.size[0] / drawing.size[1]) * H
        if size[1] is None:
            H = (drawing.size[1] / drawing.size[0]) * W
        drawing = drawing.resize((int(round(W)), int(round(H))), Image.ANTIALIAS)
        
        # Determine the placement position
        X_center = center[0] * self.W
        Y_center = center[1] * self.H
        left = int(round(X_center - W / 2))
